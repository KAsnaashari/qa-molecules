...
def E(coords, s):
    return e_perp + (e_z - e_perp) * s if s >=0 else e_perp
...
for stoq in [False, True]:
    ...
    tlist = np.linspace(-1.0 * args['tmax'], 1.0 * args['tmax'], 401)
    ...
    save_file = f'results/result_2q_1mol{"_stoq" if stoq else ""}_afm_{int(tmax * 1e4)}'
