...
solutions_q = np.array(['0101', '1010'])
...
#         else:
#             c1 = qt.tensor(c1, o1)
#     c1 = qt.Qobj(c1[conserved_states][:, conserved_states])
#     z_terms2.append(c1)
...
#             c2 = qt.tensor(c2, o2)
#     c1 = qt.Qobj(c1[conserved_states][:, conserved_states])
#     c2 = qt.Qobj(c2[conserved_states][:, conserved_states])
#     isings.append(c1)
#     c2 += c2.dag()
#     c2 /= 2
#     exchanges.append(c2)
...
for tmax in tmax_list:
    ...
    save_dir = f'{scratch_dir}/result_1d{i1 * i2}_afm_{int(tmax * 1e4)}'
    mkdir(save_dir)
    save_file = f'{save_dir}/result_1d{i1 * i2}_afm_{int(tmax * 1e4)}'
    # save_file = f'results/result_1d{i1 * i2}_afm_{int(tmax * 1e4)}_test'
