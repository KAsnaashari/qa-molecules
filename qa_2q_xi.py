...
coords = np.array([[0, 0, 0], [0, 0, 600e-9], [600e-9, 0, 0], [600e-9, 0, 600e-9]])
...
def Xi(coords, s):
    ...
    if (coords[0] == 0) and (coords[1] == 0):
        return np.array([0, 0, 0 * e]) * (1 - s)
    if (coords[0] == 600e-9) and (coords[1] == 0):
        return np.array([0, 0, 1 * e]) * (1 - s)
...
print(hs, deltas, j_z, j_perp)
hs, deltas, j_z, j_perp = l.calc_couplings_qubits_2q(E2, B, Xi1)
print(hs, deltas, j_z, j_perp)
...
#         'tmax': tmax
#     }
#     tlist = np.linspace(0.0 * args['tmax'], 1.0 * args['tmax'], 100)
...
#     np.savez(f'result_2q_xi_afm_{int(tmax * 1e4)}.npz', evals_mat=evals_mat, P_mat=P_mat, sol_over=sol_over, val_over=val_over)
#     qt.fileio.qsave(result, f'result_2q_xi_afm_{int(tmax * 1e4)}')