...
coords = np.array([[0, 0, 0], [0, 500e-9, 0], [0, 1000e-9, 0], [0, 1500e-9, 0]])
# coords = np.array([[0, 0, 0], [0, 0, 500e-9], [0, 0, 1000e-9], [0, 0, 1500e-9]])

# valid_states_q = np.array(['00', '01', '10', '11'])
# valid_states = np.array(list(map(lambda s: qubits_to_mols(s, qubits), valid_states_q)))
solutions_q = np.array(['0110'])
...
def E(coords, s):
    return e_perp + (e_z - e_perp) * np.power(s, 2) if s >=0 else e_perp
...
def MW(coords, s, l: Lattice):
    ...
    # w = biases[1] + 1/2 * s**2 * (-600)
    w = biases[1] + 1/2 * (np.tanh(5 * s - 1.5) + 1) * (-100) if s >= 0 else biases[1]
    if s < 0:
        e = 0.4 - (s + 1) * 0.25
    else:
        e = 0.15 - s * 0.1
...
couplings, biases = l.calc_couplings_biases_lattice(E1, B1, Xi1)
print(biases - biases[0])
couplings, biases = l.calc_couplings_biases_lattice(E2, B2, Xi2)
print(biases - biases[0])
...
#         'stoq': stoq
#     }
#     tlist = np.linspace(-0.5 * args['tmax'], 1.0 * args['tmax'], 401)
...
#     # save_file = f'{scratch_dir}/result_3q_1mol{"_stoq" if stoq else ""}_afm_{int(tmax * 1e4)}'
#     save_file = f'results/result_4q_1mol{"_stoq" if stoq else ""}_afm_{int(tmax * 1e4)}'
